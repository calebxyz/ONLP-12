...
class Submission(SubmissionSpec12):
    ...
    def getProb(self, pTag, tag, word):
        ...
        emitProb = 0
        if (tag, word) in self.e:
            emitProb = ((self.e[(tag, word)] + self.delta) / (self.e[(tag)] + self.delta * self.uniqueStates))
        else:
            emitProb = self.delta / (self.e[(tag)] + self.delta * self.uniqueStates)

        if (pTag, tag) in self.t:
            return (self.t[(pTag, tag)] / self.t[(pTag)]) * emitProb  # P("s|prev s") * P("w|s")
        else:
            return 0
