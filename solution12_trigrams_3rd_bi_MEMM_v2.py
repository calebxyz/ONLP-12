...
class Submission(SubmissionSpec12):
    ...
    def getProbPredArr(self, sentence):
        vect = self.vectorize(sentence, 1)
        pred = self.LRM.predict_proba([vect])[0]
        result = [0] * len(self.tag_setIndexDic)
        for idx in range(len(pred)):
    ...
    def viterbi(self, sentence):
        ...
        for s in range(0, len(self.p_tag_set)):
            self.viterbiMat[s][0] = self.getProbPred([("SOS","SOS"), (sentence[0], self.p_tag_set[s])]) #self.getProb(("SOS",) + self.p_tag_set[s], sentence[0])  #P("s|<s>") * P("w|s")
            self.backpointerMat[s][0] = 0

        probArr = self.getProbPredArr([("SOS", "SOS"), (sentence[0], self.p_tag_set[s])])
        for idx in range(len(probArr)-1):
            assert(self.viterbiMat[idx][0] == probArr[idx])

        for t in range(1, len(sentence)):
            mat = numpy.zeros(shape=(len(self.p_tag_set), len(self.p_tag_set)))
            for s in range(0, len(self.p_tag_set)):
                probArr = self.getProbPredArr([(self.p_tag_set[s][-1], self.p_tag_set[s][-1]), (sentence[t], (None,))])
                for i in range(len(self.p_tag_set)):
                    mat[s, i] = self.viterbiMat[s][t - 1] * probArr[i]

            for s in range(0, len(self.p_tag_set)):
                self.backpointerMat[s][t], self.viterbiMat[s][t] = self.maxViterbiArr(s, mat[:, s])
